      ...
      donuts.forEach(({ config, particles }) => {
        ...
        particles.forEach(p => {
          ...
          if (drawScale > 0) {
            ...
            ctx.globalAlpha = Math.max(0.4, (z2 + config.r) / (config.r * 2)) * (1 - window.scrollY / 1000);
            
            // 添加粒子发光效果
            ctx.shadowColor = config.color;
            ctx.shadowBlur = 20;
            
            ctx.fillRect(x2 * drawScale + canvas.width / 2 + config.offsetX, y1 * drawScale + canvas.height / 2 + config.offsetY, drawScale * 3, drawScale * 3);
            ctx.restore();
          }
        });
      });
